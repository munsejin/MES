...
namespace MES.CLS
{
    class wnProcCon
    {
        ...
        public void prod_raw_out(string lot_no, string raw_mat_cd, double out_amt) 
        {
            ...
            {
                System.Console.WriteLine("프로시저 에러" + num.ToString());
                MessageBox.Show("프로시저 에러" + num.ToString());
                return;
            }
        }
        ...
        public void prod_half_out2(string lot_no, string half_item_cd, double out_amt)
        {
            wnDmProc wDmProc = new wnDmProc();
            int num = wDmProc.sp_half_out2(lot_no, half_item_cd, out_amt);
