            ...
            {               
                ...
                try
                {
                    if (rs_order_grid.Rows.Count > 0)
                    {
                        int chk = 0;
                        //체크박스 재 정의 
                        int[] t = new int[rs_order_grid.Rows.Count];   
                        ...
                        //        for (int i = 0; i < rs_order_grid.Rows.Count; i++)
                        //        {
                        //            if ((bool)rs_order_grid.Rows[i].Cells[0].Value == false) //체크 하지 않을 경우 제외
                        //            {
                        //                sb2.AppendLine(" and  C.RAW_MAT_CD != '" + rs_order_grid.Rows[i].Cells["RAW_MAT_CD"].Value.ToString() + "' ");
                        //            }
                        //            else if ((bool)rs_order_grid.Rows[i].Cells[0].Value == true && rs_order_grid.Rows[i].Cells["RS_AMT"].Value.ToString().Equals("0")) //체크는 했는데 발주량이 0일 경우 
                        //            {
                        //                cnt++;
                        //                sb2.AppendLine(" and  C.RAW_MAT_CD != '" + rs_order_grid.Rows[i].Cells["RAW_MAT_CD"].Value.ToString() + "' ");
                        //                w_sb.AppendLine(" 원부재료명: '" + rs_order_grid.Rows[i].Cells["RAW_MAT_CD"].Value.ToString() + "' , 규격: '" + rs_order_grid.Rows[i].Cells["RAW_MAT_CD"].Value.ToString()+"' ");
                        //            }
                        //        }
                        //        soyoLogic(sb2);
                        //    }
                        ...
                        //테스트
                        //rs_order_grid2.Rows.Clear();
                        //for (int i = 0; i < chk; i++)
                        //{
                        //    rs_order_grid2.Rows.Add();
                        //    rs_order_grid2.Rows[i].Cells["PUR_CUST_NM2"].Value = rs_order_grid.Rows[list[i]].Cells["PUR_CUST_NM"].Value;
                        //    rs_order_grid2.Rows[i].Cells["RAW_MAT_NM2"].Value = rs_order_grid.Rows[list[i]].Cells["RAW_MAT_NM"].Value;
                        //    rs_order_grid2.Rows[i].Cells["SPEC2"].Value = rs_order_grid.Rows[list[i]].Cells["SPEC"].Value;
                        //    rs_order_grid2.Rows[i].Cells["TOTAL_AMT2"].Value = rs_order_grid.Rows[list[i]].Cells["TOTAL_AMT"].Value;
                        //    rs_order_grid2.Rows[i].Cells["BAL_STOCK2"].Value = rs_order_grid.Rows[list[i]].Cells["BAL_STOCK"].Value;
                        //    rs_order_grid2.Rows[i].Cells["REAL_AMT2"].Value = rs_order_grid.Rows[list[i]].Cells["REAL_AMT"].Value;
                        //    rs_order_grid2.Rows[i].Cells["RS_AMT2"].Value = rs_order_grid.Rows[list[i]].Cells["RS_AMT"].Value;
                        //    rs_order_grid2.Rows[i].Cells["PRICE2"].Value = rs_order_grid.Rows[list[i]].Cells["PRICE"].Value;
                        //    rs_order_grid2.Rows[i].Cells["TOTAL_MONEY2"].Value = rs_order_grid.Rows[list[i]].Cells["TOTAL_MONEY"].Value;
                        //    rs_order_grid2.Rows[i].Cells["RAW_MAT_CD2"].Value = rs_order_grid.Rows[list[i]].Cells["RAW_MAT_CD"].Value;
                        //    rs_order_grid2.Rows[i].Cells["PUR_CUST_CD2"].Value = rs_order_grid.Rows[list[i]].Cells["PUR_CUST_CD"].Value;
                        //    rs_order_grid2.Rows[i].Cells["CUST_NUM2"].Value = rs_order_grid.Rows[list[i]].Cells["CUST_NUM"].Value;
                        //    rs_order_grid2.Rows[i].Cells["UNIT_CD2"].Value = rs_order_grid.Rows[list[i]].Cells["UNIT_CD"].Value;                            
                        ...
                        {
                            StringBuilder sb = new StringBuilder();
                            sb.AppendLine(chk + "개의 원부재료를 선택하셨습니다.");
                            sb.AppendLine("원부재료는 각 거래처별로 나눠서 저장됩니다.");
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        dgv.Rows[i].Cells["ITEM_CNT"].Value = dt.Rows[i]["ITEM_NM"].ToString();
