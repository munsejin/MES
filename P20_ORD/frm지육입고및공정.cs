...
namespace MES.P20_ORD
{
    public partial class frm지육입고및공정 : Form
    {
        ...
        private void Meat_Input()
        {
            try
            {
                ...
                if (chk_input_yn.Checked == true)
                {
                    ...
                    sb.AppendLine("※ 공정 완료 확인 ※");
                    sb.AppendLine("공정 완료에 체크한 경우 ");
                    sb.AppendLine("저장 이후 수정이 불가능합니다 (삭제 가능) ");
                    sb.AppendLine("모든 산출물 입력을 마치고 진행하여 주십시오 ");
                ...
                if (lbl_input_gbn.Text.Equals(""))
                {
                    rsNum = wDm.insert_Meat_Input(
                                    ...
                                    , cmb_corigin.SelectedValue.ToString()
                                    , sourceRawGrid
                                    );
                }
                else
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        dgv.Rows[i].Cells[12].Value = dt.Rows[i]["COUNTRY_ORIGIN"].ToString();
        ...
        private void input_detail(DataGridView dgv, DataGridViewCellEventArgs e)
        {
            ...
            cmb_corigin.Text = dgv.Rows[e.RowIndex].Cells[12].Value.ToString();
