    ...
    public partial class frm작업공정등록 : Form
    {
        ...
        private void getSetting() 
        {
            ...
            for (int i = 0; i < max_flow_cnt; i++) 
            {
                ...
                dgv[i].Columns.Add("INST_AMT" + (i + 1).ToString("00"), "완료"); //3
                ...
                dgv[i].Columns.Add("POOR_AMT" + (i + 1).ToString("00"), "불량수"); //index 6
                ...
                dgv[i].Columns[3].Width = 48;
                dgv[i].Columns[4].Width = 48;
                dgv[i].Columns[5].Width = 50;
                dgv[i].Columns[6].Width = 50;
        ...
        private void flow_detail_logic()
        {
            ...
            sb.AppendLine(" and B.ITEM_IDEN_YN = 'Y' ");
            dt = wDm.fn_Item_Flow_Iden_Cnt(sb.ToString());
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                if (int.Parse(dt.Rows[0]["cnt"].ToString()) == 0)
                {
                    ...
                    sb.AppendLine(" and A.ITEM_CD = '" + txt_item_cd.Text.ToString() + "' ");
                    dt = wDm.fn_Item_Flow_Iden_Cnt(sb.ToString());
                    lastFlow = int.Parse(dt.Rows[0]["cnt"].ToString());

                    MessageBox.Show("제품 공정에 제품식별표 발행이 없습니다 . 마지막 공정(" + lastFlow +"단계)에서 발행하겠습니다. ");
            ...
            if (flow_cnt > 0)
            {
                if (max_flow_cnt<flow_cnt)
                {
                    MessageBox.Show("작업 공정이 너무 많습니다 .");
                    return;
                }
            ...
            if (complete_YN=="진행중")  //  공정 상태가 S : 진행중일때 
            {

                //this.lbl_title.Text = "작업공정등록 - 수정";
                this.btnFlow.Enabled = true;
                this.btnFlow.Visible = true;
                ...
                if (dt2 != null && dt2.Rows.Count > 0)
                {
                    ...
                    if (flow_dt != null && flow_dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < flow_dt.Rows.Count; i++)
                        {
                            ...
                            {
                                 dgv[idx].Rows[rowIdx].Cells[8].Value=true;
                              
                            }
                            else
                            {
                                dgv[idx].Rows[rowIdx].Cells[8].Value = false;
                            }
                          
                         //   dgv[idx].Rows[rowIdx].Cells[dgv[0].ColumnCount - 1].Value = false;

                            DataGridViewRow clonedRow = (DataGridViewRow)dgv[idx].Rows[rowIdx].Clone();


                                switch (i)
                            {
                                case 0:
                                    Debug.WriteLine(flow_dt.Rows[i]["미투입량"].ToString());
                                  dgv[idx].Rows.Add(clonedRow);
                                  dgv[idx].Rows[dgv[idx].Rows.Count-1].Cells[3].Value = Math.Round(double.Parse(flow_dt.Rows[i]["미투입량"].ToString()));
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[0].Value = flow_dt.Rows[i]["LOT_NO"].ToString();
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[1].Value = flow_dt.Rows[i]["LOT_SUB"].ToString();
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[2].Value = flow_dt.Rows[i]["F_SUB_DATE"].ToString();
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[7].Value = flow_dt.Rows[i]["F_STEP"].ToString();
                                

                                    break;
                                default:
                                    Debug.WriteLine(double.Parse(flow_dt.Rows[i - 1]["F_SUB_AMT"].ToString()) - double.Parse(flow_dt.Rows[i]["INPUT_AMT"].ToString()));

                                 
                                  dgv[idx].Rows.Add(clonedRow);
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[3].Value = Math.Round(double.Parse(flow_dt.Rows[i - 1]["F_SUB_AMT"].ToString()) - double.Parse(flow_dt.Rows[i]["INPUT_AMT"].ToString()));
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[0].Value = flow_dt.Rows[i]["LOT_NO"].ToString();
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[1].Value = flow_dt.Rows[i]["LOT_SUB"].ToString();
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[2].Value = flow_dt.Rows[i]["F_SUB_DATE"].ToString();
                                  dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[7].Value = flow_dt.Rows[i]["F_STEP"].ToString();
        ...
        private void btn_work_srch_Click(object sender, EventArgs e)
        {
            ...
            if (frm.sCode != "")
            {
                ...
                complete_YN = frm.dgv.Rows[0].Cells["complete_YN"].Value.ToString();
        ...
        private void btn_logic(int idx) 
        {
            ...
            {
                if ((bool)dgv[idx].Rows[i].Cells[8].Value)
                {
                    dgv[idx + 1].Rows[i].Cells[0] = dgv[idx].Rows[i].Cells[0];
                    dgv[idx + 1].Rows[i].Cells[1] = dgv[idx].Rows[i].Cells[1];
                    dgv[idx + 1].Rows[i].Cells[2] = dgv[idx].Rows[i].Cells[2];
                    dgv[idx + 1].Rows[i].Cells[3] = dgv[idx].Rows[i].Cells[3];
                }
            }
            ...
            for (int i = 0; i < dgv[idx].Rows.Count; i++) 
            {
                ...
                if ((bool)dgv[idx].Rows[i].Cells[col_cnt].Value == true) 
                {
                    if (dgv[idx + 1].Rows.Count > 0) //해당 공정 테이블의 LotNo와 LotSub가 다음 공정 테이블에 Lotno와 Lotsub에 있을 경우
                    {
                        ...
                        double poor_amt = double.Parse((string)dgv[idx].Rows[i].Cells[6].Value);
                        ...
                        //double rs_flow_amt = flow_amt;
                        dgv[idx].Rows[i].Cells[3].Value = (decimal.Parse((flow_amt).ToString())).ToString("#,0.######");
                        ...
                        //dgv[idx + 1].Rows[dgv[idx + 1].Rows.Count - 1].Cells[3].Value = (decimal.Parse((rs_flow_amt).ToString())).ToString("#,0.######");
                        dgv[idx + 1].Rows[dgv[idx + 1].Rows.Count - 1].Cells[4].Value = "0";
                        dgv[idx + 1].Rows[dgv[idx + 1].Rows.Count - 1].Cells[6].Value = "0";


                        Debug.WriteLine(dgv[idx].Rows[i].Cells[3].Value.ToString()); //투입수량
                        Debug.WriteLine(double.Parse(dgv[idx + 1].Rows[dgv[idx + 1].Rows.Count - 1].Cells[3].Value.ToString()));  //다음 투입수량(loss랑 불량율 뺀)

                         realAMT=double.Parse(dgv[idx + 1].Rows[dgv[idx + 1].Rows.Count - 1].Cells[3].Value.ToString()) - double.Parse(dgv[idx].Rows[i].Cells[3].Value.ToString());
                        Debug.WriteLine(realAMT);
            ...
            //if (idx == 0)
            //{
            //    double firstAMT = double.Parse(txt_inst_amt.Text.ToString().Replace(",", "")) - double.Parse(dgv[idx + 1].Rows[dgv[idx + 1].Rows.Count - 1].Cells[3].Value.ToString());

            //    if (firstAMT != 0)
            //    {
            //        dgv[idx].Rows.Add();
            //        dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[3].Value = firstAMT.ToString();
            //    }
            //}
            //else
            //{
            //    double firstAMT = double.Parse(dgv[idx - 1].Rows[0].Cells[3].Value.ToString().Replace(",", "")) - double.Parse(dgv[idx - 1].Rows[0].Cells[4].Value.ToString().Replace(",", "")) - double.Parse(dgv[idx - 1].Rows[0].Cells[6].Value.ToString().Replace(",", "")) - double.Parse(dgv[idx].Rows[0].Cells[3].Value.ToString()) + double.Parse(dgv[idx].Rows[0].Cells[4].Value.ToString()) + double.Parse(dgv[idx].Rows[0].Cells[6].Value.ToString());
            //    if (firstAMT != 0)
            //    {
            //        dgv[idx].Rows.Add();
            //        dgv[idx].Rows[dgv[idx].Rows.Count - 1].Cells[3].Value = firstAMT.ToString();
        ...
        private void workComp01_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
           
        }

        private void  checkchange(DataGridView 데이터뷰)
        {
