...
namespace MES.P30_SCH
{
    public partial class frm작업지시서_모니터링 : Form
    {
        ...
        private FormWindowState Maximized;
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        itemOutGrid.Rows[i].Cells["설비명"].Value = dt.Rows[i]["LINE_NM"].ToString();
                        itemOutGrid.Rows[i].Cells["계획번호"].Value = dt.Rows[i]["PLAN_NUM"].ToString();
                        itemOutGrid.Rows[i].Cells["원자재투입"].Value = dt.Rows[i]["RAW_OUT_YN"].ToString();

                        if ( itemOutGrid.Rows[i].Cells["원자재투입"].Value.ToString()=="N")
                        {
                            itemOutGrid.Rows[i].DefaultCellStyle.ForeColor = Color.Chocolate;
                        ...
                        else
                        {
                            ...
                            itemOutGrid.Rows[i].Cells["ITEM_NM"].Style.ForeColor = Color.Black;
                        }
        ...
        public DataTable fn_작업지시서_모니터링_List(string sDayFrom, string sDayTo)
        {
            ...
            sb.AppendLine("     ,ISNULL((SELECT MAX(F_SUB_DATE) FROM F_WORK_FLOW_DETAIL AS K WHERE K.LOT_NO = A.LOT_NO), '공정 미진행') AS 공정일자 ");
            ...
            sb.AppendLine(" left join N_LINE_CODE as G on G.LINE_CD=A.LINE_CD ");
        ...
        {
            lbl_title.Text = Common.p_strCompNm + "   작업지시 모니터링     " + DateTime.Now.ToString("yyyy.MM.dd    HH:mm:ss");
