...
namespace MES.P50_QUA
{
    public partial class frm공정검사기준등록 : Form
    {
        ...
        private void frm공정검사기준등록_Load(object sender, EventArgs e)
        {
            ...
            tbStanControl.TabPages.Remove(tabPage2);
        }
        ...
        private void gridSetting(conDataGridView dgv, string met_cd, string met_nm, DataGridView del_dgv)
        {
            ...
            dgv.Columns["LOWER_SIZE"].Visible = false;
            dgv.Columns["UPPER_SIZE"].Visible = false;
            dgv.Columns["LOWER_SELF"].Visible = false;
            dgv.Columns["UPPER_SELF"].Visible = false;
        ...
        private void item_flow_list()
        {
            try
            {
                ...
                if (chk_isnull_chk.Checked)  //미등록인것만 보고싶을때 
                {
                    sb.AppendLine("and D.FLOW_CD is null ");

                }
                dt = wDm.fn_Item_List_검사용(sb.ToString());
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        if (dt.Rows[i]["등록"].ToString() != "등록")
                        {
                            ...
                            dataItemGrid.Rows[i].Cells["규격"].Value = dt.Rows[i]["SPEC"].ToString();
                            dataItemGrid.Rows[i].Cells["ITEM_GUBUN_NM"].Value = dt.Rows[i]["ITEM_GUBUN_NM"].ToString();
                            dataItemGrid.Rows[i].Cells["CHK_YN"].Value = dt.Rows[i]["등록"].ToString();  // 등록 Y 미등록 N 
                            dataItemGrid.Rows[i].Cells["FLOW"].Value = dt.Rows[i]["FLOW_NM"].ToString(); //공정 종류 
                            dataItemGrid.Rows[i].Cells["공정코드"].Value = dt.Rows[i]["공정코드"].ToString(); //공정 종류 
        ...
        private void dataItemGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                cmb_item_flow.SelectedItem = true;

                FlowChkGrid.Rows.Clear();
                lbl_title.Text = "공정검사기준 ";
                ...
                try
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        cmb_item_flow.SelectedValue = dgv.Rows[e.RowIndex].Cells["공정코드"].Value;
                    }
                    else
                    {
