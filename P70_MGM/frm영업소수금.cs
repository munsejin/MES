...
wnLog.writeLog(wnLog.LOG_ERROR, ex.Message + " - " + ex.ToString());Popup.pop오류리포트 msg = new Popup.pop오류리포트(ex.Message + " - " + ex.ToString());
        ...
        private void btn거래처_Click(object sender, EventArgs e)
        {
            ...
            wConst.call_popRef_Cust("", txt거래처코드, txt거래처명, "0","거래처");
            if (txt거래처코드.Text != "")
            {
        ...
        private void btnS거래처_Click(object sender, EventArgs e)
        {
            ...
            wConst.call_popRef_Cust("", txtS거래처코드, txtS거래처명, "","거래처");
            //if (txtS거래처코드.Text != "")
            //{
        ...
        private void txtS거래처_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (bPopSrch == true)
                {
                    wConst.call_popRef_Cust(txtS거래처명.Text, txtS거래처코드, txtS거래처명, "","거래처");
                    //get_Srch_Cust_Info(txtS거래처코드.Text, txtS거래처명);
                }
