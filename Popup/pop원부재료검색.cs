...
namespace MES.Popup
{
    public partial class pop원자재검색 : Form
    {
        private int iCnt;
        ...
        public pop원자재검색(String CUST_CD)
        {
            InitializeComponent();
            strCUST_CD = CUST_CD;
        }

        private void pop원자재검색_Load(object sender, EventArgs e)
        {
            string sqlQuery = "";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (s자재입고.Equals("1"))
            {
                sb.AppendLine("inner join F_ORDER_DETAIL C on A.RAW_MAT_CD = C.RAW_MAT_CD INNER join F_ORDER B ON C.ORDER_DATE = B.ORDER_DATE AND C.ORDER_CD = B.ORDER_CD AND B.COMPLETE_YN = 'N'");
            }
            ...
            if (gubun == 1)
            {
                sb.AppendLine("and B.CUST_CD = '" + strCUST_CD + "'");
                dt = wDm.fn_Raw_List(sb.ToString(), "2");
