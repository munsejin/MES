...
namespace MES.Popup
{
    public partial class pop제품엑셀입력 : Form
    {
        ...
        private void pop제품엑셀입력_Load(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < excelGrid.Rows.Count; i++)
                    {
                        ...
                        if (excelGrid.Rows[i].Cells[6].Value == null || excelGrid.Rows[i].Cells[6].Value.ToString().Equals(""))
                        {
                            excelGrid.Rows[i].Cells[6].Value = "0";
                        }

                        if (excelGrid.Rows[i].Cells[0].Value == null || excelGrid.Rows[i].Cells[0].Value.ToString().Equals("")
                            || excelGrid.Rows[i].Cells[1].Value == null || excelGrid.Rows[i].Cells[1].Value.ToString().Equals("")
                            || excelGrid.Rows[i].Cells[2].Value == null || excelGrid.Rows[i].Cells[2].Value.ToString().Equals("")
                            || excelGrid.Rows[i].Cells[8].Value == null || excelGrid.Rows[i].Cells[8].Value.ToString().Equals("")
        ...
        private void btn_insert_Click(object sender, EventArgs e)
        {
            if (excelGrid.Rows.Count > 0)
            {
                ...
                {
                    wnDm wDm = new wnDm();
                    int rsNum = wDm.fn_insert_item_cd_from_Excel(excelGrid);
                    ...
                    {
                        MessageBox.Show("저장에 실패하였습니다. 제품코드가 중복되지 않았는지 확인하여주십시오");
                        return;
                    }
                    else
                    {
        ...
        private void CopyExcel_Templete(string targetPath)
        {
            try
            {
                string fileName = "MES_제품입력양식.xlsx";
