...
namespace MES.S20_BILL
{
    public partial class frm지출등록 : Form
    {
        ...
        private void out_delete()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                MessageBox.Show("이미 출고된 원자재가 있기 때문에 삭제할 수 없습니다.");
                return;
            }
            int rsNum = wDm.OutDelete(
        ...
        public void dgvsrchlist()
        {
            ...
            dgv_srch.Rows.Clear();
            string condition = "WHERE B.PAY_DATE >= '" + dtp_start.Text.ToString() + "' and  B.PAY_DATE <= '" + dtp_end.Text.ToString() + "'";
            ...
            dt = wDm.OutSrchSelect(condition);
        ...
        {
            dgv_main.Rows.Add();
            dgv_main.Rows[dgv_main.Rows.Count - 1].Cells["JUKYO"].Value = "";
        ...
        private void dgv_srch_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > -1)
            {
                ...
                dt = wDm.OutMainSelect("WHERE A.PAY_DATE ='" + dtp_outdate.Text + "'  and A.PAY_CD = '" + txt_num.Text + "'  ");
                ...
                dt = wDm.OutMainSelect("WHERE B.STAFF_CD = '" + cmb_staff.SelectedValue.ToString() + "'   ");
        ...
        private void init_ComboBox()
        {
            ...
            wConst.GridComboBox_Read_Blank(ID, sqlQuery);

            GUBUN.ValueMember = "코드";
            GUBUN.DisplayMember = "명칭";
            sqlQuery = comInfo.querySooGubun();
            wConst.GridComboBox_Read_Blank(GUBUN, sqlQuery);
